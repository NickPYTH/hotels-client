...
export const ExtrasReportModal = (props: ModalProps) => {
    ...
    const confirmHandler = () => {
        ...
        if (dateRange && selectedHotelId) {
            tmpButton.href = `${host}/hotels/api/report/getExtrasReport?hotelId=${selectedHotelId}&dateStart=${dateRange[0].format("DD-MM-YYYY")}&dateFinish=${dateRange[1].format("DD-MM-YYYY")}`;
            tmpButton.click();
        } else {
    ...
    return (
        <Modal title={"Информация о дополннительных услугах"}
               open={props.visible}
               onOk={confirmHandler}
