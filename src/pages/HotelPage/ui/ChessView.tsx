...
export const ChessView = (props: ModalPros) => {
    ...
    const [cellColorDeadline] = useState(() => {
        return localStorage.getItem('cellColorDeadline') ?? '#CE6969';
    });
    const [cellColorGuestDeadline] = useState(() => {
            ...
            render: (val: any, record: any) => {
                ...
                if (val) {
                    ...
                    if (val.split('||').length === 1) { // Если ячейка с одним жильцом
                        ...
                        let isCheckouted = val.split('#')[0].split('&')[7];
                        ...
                        return (<Flex key={record.id} vertical={false} style={{background: isWeekend ? cellBackgroundColor : 'inherit', height: 31}}>
                            <div onClick={() => {
                                ...
                                marginBottom: 3,
                                cursor: 'pointer',
                                background: (isReservation == 'true' && dayjs().isAfter(dayjs(el, 'DD-MM-YYYY'))) ? cellColorDeadline :
                                    (isReservation == 'false' && dayjs().isAfter(dayjs(dateTime[1], 'DD-MM-YYYY')) && isCheckouted == 'false') ? cellColorGuestDeadline :
                                    male ? cellsMaleColor : cellsFemaleColor,
                        ...
                        let personLeftDateTime: string[] = personLeft.split('#')[0].split('&')[1].split(' :: ');
                        ...
                        let personLeftIsCheckouted = personLeft.split('#')[0].split('&')[7];
                        ...
                        let personRightDateTime: string[] = personRight.split('#')[0].split('&')[1].split(' :: ');
                        ...
                        let personRightIsCheckouted = personRight.split('#')[0].split('&')[7];
                            ...
                            <>
                                <div onClick={() => {
                                    ...
                                    setSelectedDate(dayjs(personLeftDates.split(" :: ")[1].replace("12:00", "11:59"), "DD-MM-YYYY HH:mm"));
                                     ...
                                     style={{
                                         ...
                                         marginBottom: 3,
                                         cursor: 'pointer',
                                         background: (personLeftIsReservation == 'true' && dayjs().isAfter(dayjs(el, 'DD-MM-YYYY'))) ? cellColorDeadline :
                                             (personLeftIsReservation == 'false' && dayjs().isAfter(dayjs(personLeftDateTime[1], 'DD-MM-YYYY')) && personLeftIsCheckouted == 'false') ? cellColorGuestDeadline :
                                                 personLeftMale ? cellsMaleColor : cellsFemaleColor,
                                    ...
                                    <Flex justify="center" align="center">
                                        <Popover placement={'bottom'} title={`${fioPersonLeft}`} content={() => (<Flex vertical={true}>
                                            ...
                                            <div style={{paddingTop: 4, width: fioPersonLeft.length * 1.5 + 15, height: 21}}>
                                            </div>
                                        </Popover>
                                    </Flex>
                                </div>
                                ...
                                <div
                                    ...
                                    style={{
                                        ...
                                        color: fontColor,
                                        fontSize,
                                        background: (personRightIsReservation == 'true' && dayjs().isAfter(dayjs(el, 'DD-MM-YYYY'))) ? cellColorDeadline :
                                            (personRightIsReservation == 'false' && dayjs().isAfter(dayjs(personRightDateTime[1], 'DD-MM-YYYY')) && personRightIsCheckouted == 'false') ? cellColorGuestDeadline :
                                                personRightMale ? cellsMaleColor : cellsFemaleColor,
                                    ...
                                    <Flex justify="center" align="center">
                                        <Popover placement={'bottom'} title={`${fioPersonRight}`} content={() => (<Flex vertical={true}>
                                            ...
                                            <div style={{paddingTop: 4, width: Math.abs(personRightPercent) + addedPixel * (Math.abs(personRightPercent) / 100), height: 21}}>
                                                {startCell &&
    ...
    const updateCurrentMonthHandler = (value: string) => {
        ...
        switch (value) {
            ...
            case "Сентябрь":
                ...
                break;
            case "Октябрь":
                props.setChessDateRange([dayjs('01-10-2025', 'DD-MM-YYYY'), dayjs('31-10-2025', 'DD-MM-YYYY')])
                break;
            case "Ноябрь":
