                ...
                <Flex style={{width: '100%'}} vertical align={'center'}>
                    <Button style={{marginTop: 15, width: 280}} onClick={() => {
                        ...
                        if (props.guest) {
                            ...
                            tmpButton.href = `${host}/hotels/api/report/getCheckoutReport?id=${props.guest.id}&periodStart=${periodStart}&periodEnd=${periodEnd}&format=pdf`;
                            tmpButton.click();
                        }
                    }}>Отчетный документ</Button>

                    <Popconfirm title={"Вы уверены?"} onConfirm={() => {
                        if (props.guest.id) checkout(props.guest.id);
                    }}>
